use crate::{Value, Params, xmlfmt::{ToXml}};
use crate::xmlfmt::{XmlError, XmlResult, FmtError};
...
impl ToXml for MethodResponse {
    fn to_xml(&self) -> XmlResult<String> {
        to_string(self)
            .map_err(|e| XmlError::Format(FmtError::Encoding(e.to_string())))
    }
}
...
pub(crate) mod tests {
    ...
    pub fn ser_and_de_response_value(value: MethodResponse) {
        ...
        assert!(data.is_ok(), "Unable to serialize Response value! {}", data.unwrap_err());
